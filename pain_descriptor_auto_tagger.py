...
def assign_experience_tags(text):
    ...
    if any(word in text for word in ["burn", "stab", "shoot", "tight", "crawl", "crush", "fire", "pierce", "pain", "hot", "sting"]):
        tags.append("sensory")
    if any(word in text for word in ["monster", "witch", "frankenstein", "angry", "insane", "unravel", "losing my mind", "explode", "volcano"]):
        tags.append("affective")
    if any(word in text for word in ["always", "constant", "sudden", "gradual", "never ends", "won't stop", "creeping"]):
...
def estimate_intensity(text):
    text = text.lower()
    if any(word in text for word in ["stab", "explode", "burning", "crushed", "shooting", "severe"]):
        return "high"
    elif any(word in text for word in ["tight", "sharp", "dull", "pounding", "nagging", "throbbing"]):
        return "medium"
    elif any(word in text for word in ["flutter", "tingling", "light", "buzz", "flicker"]):
        return "low"
    return ""
...
def run_auto_tagger(input_path=INPUT_CSV, output_path=OUTPUT_CSV):
    ...
    df["metaphor_type"], df["metaphor_subtypes"] = zip(*df["feels like…"].apply(detect_metaphor_type))
